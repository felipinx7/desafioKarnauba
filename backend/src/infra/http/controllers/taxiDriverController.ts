...
export class taxiDriverController {
    ...
    async create(fastify: FastifyContextDTO) {
        ...
        const taxiDriver = await this.taxiDriverCreateUseCase.execute(data, fastify.req);
    ...
    async findAll(fastify: FastifyContextDTO) {
        ...
        fastify.res.send({ message: "Taxi Drivers found",  ...taxiDriver });
    }
    ...
    async update(fastify: FastifyContextDTO) {
        const { id } = fastify.req.params as { id: string };
        const data = await this.multipart.handleDataMultipart(fastify.req, "taxiDrivers");
        const updatedTaxiDriver = await this.taxiDriverUpdateUseCase.execute(data, id);
